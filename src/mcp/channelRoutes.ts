...
export function registerChannelRoutes(app: Express, d: ChannelRouteDeps): void {
  ...
  const configured = (res: Response): Buffer | null => {
    if (!d.secretsKey)
      res.status(503).json({ error: 'Alert channels are not configured on this deployment.' });
    return d.secretsKey;
  };
  const test = (
    kind: ChannelKind,
    ...
    d.post(
      url,
      formatTest(kind, minSeverity, {
        deliveryId: `test:${Date.now()}`,
        dashboardUrl: `${d.webUrl}/dashboard/notifications`,
        now: new Date(),
        signingSecret,
      }),
    );
  ...
  app.get('/notification-channels', d.requireIssuerSecret, async (req: Request, res: Response) => {
    ...
    try {
      const [channels, allowed] = await Promise.all([
        listChannels(d.db, ownerId),
        d.allowed(ownerId).catch(() => true),
      ]);
      res
        .status(200)
        .json({ channels: channels.map(channelView), allowed, configured: Boolean(d.secretsKey) });
    } catch (err) {
  ...
  app.post('/notification-channels', d.requireIssuerSecret, async (req: Request, res: Response) => {
    ...
    if (!KINDS.includes(kind))
      return void res.status(400).json({ error: 'kind must be slack, discord, teams or webhook.' });
    if (!SEVERITIES.includes(minSeverity))
      return void res
        .status(400)
        .json({ error: 'minSeverity must be critical, high, moderate or low.' });
    ...
    try {
      ...
      if (!(await d.allowed(ownerId).catch(() => true))) {
        return void res
          .status(403)
          .json({ error: 'Slack, Discord, Teams and webhook alerts come with the Team plan.' });
      }
      if ((await listChannels(d.db, ownerId)).length >= MAX_CHANNELS_PER_OWNER) {
        return void res
          .status(400)
          .json({ error: `An account can have up to ${MAX_CHANNELS_PER_OWNER} alert channels.` });
      }
      ...
      if (outcome.status < 200 || outcome.status >= 300) {
        return void res.status(400).json({
          error: `lurq could not post to that URL: ${outcome.error ?? `HTTP ${outcome.status}`}`,
        });
      }
      ...
      res
        .status(201)
        .json({ channel: channelView(channel), ...(signingSecret ? { signingSecret } : {}) });
    } catch (err) {
    ...
    async (req: Request, res: Response) => {
      ...
      if (body.enabled !== undefined) {
        if (typeof body.enabled !== 'boolean')
          return void res.status(400).json({ error: 'enabled must be true or false.' });
      ...
      if (body.minSeverity !== undefined) {
        if (!SEVERITIES.includes(body.minSeverity as Severity))
          return void res
            .status(400)
            .json({ error: 'minSeverity must be critical, high, moderate or low.' });
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (!channel)
          return void res.status(404).json({ error: 'No such channel for this account.' });
        const url = open(channel.urlCiphertext, key, ownerId);
        const secret = channel.signingSecretCiphertext
          ? open(channel.signingSecretCiphertext, key, ownerId)
          : null;
        ...
        await updateChannel(
          ...
          ownerId,
          id,
          ok ? { lastDeliveredAt: new Date(), lastError: null } : { lastError: outcome.error },
        );
  ...
  app.delete(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!id) return;
      try {
        if (!(await removeChannel(d.db, ownerId, id)))
          return void res.status(404).json({ error: 'No such channel for this account.' });
