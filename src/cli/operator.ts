...
export function registerOperatorCommands(program: Command): void {
  ...
  billing
    .command('grant')
    .argument('<ownerId>', 'the Clerk user id from the dashboard')
    .description('give an account a paid plan by hand, with no payment processor')
    .option('--tier <tier>', 'pro | enterprise', 'pro')
    .option('--months <n>', 'how long the grant runs before it lapses', '1')
    .option('--note <text>', 'invoice number, deal, whoever asked')
    .action(async (ownerId: string, opts: { tier: string; months: string; note?: string }) => {
      const { runBillingGrant } = await import('./billing');
      await runBillingGrant(ownerId, opts);
    });
  billing
    .command('revoke')
    .argument('<ownerId>', 'the Clerk user id')
    .description('end a plan now, dropping the account to free')
  ...
  billing
    .command('grants')
    .description('list hand-granted plans, soonest to lapse first')
    .action(async () => {
      const { runBillingGrants } = await import('./billing');
      await runBillingGrants();
    });
  billing
    .command('status')
