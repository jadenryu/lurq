...
function pinLine(p: import('./remote').RemotePin): string {
  const drift = [p.contractChanged && 'tools changed', p.authChanged && 'sign-in changed']
    .filter(Boolean)
    .join(', ');
  const open = p.openChanges
    ? ` · ${p.openChanges} open change(s)${p.worstOpen ? `, worst ${p.worstOpen}` : ''}`
    : '';
...
export async function runMcpPin(
  server: string,
  ...
  if (!pin)
    return console.log(yellow(`${server} was pinned, but lurq could not read the pin back.`));
  console.log(`${green('pinned')} ${bold(pin.url)}${pin.note ? dim(`  "${pin.note}"`) : ''}`);
  console.log(
    dim(`status ${pin.status ?? 'not probed yet'} · pinned ${pin.pinnedAt.slice(0, 10)}`),
  );
  console.log(
    dim(
      'lurq alerts this account when its tools, its sign-in path or its availability change. Re-run to approve a change you reviewed.',
    ),
  );
}
...
export async function runMcpUnpin(server: string): Promise<void> {
  ...
  console.log(
    (await unpinMcpServer(server))
      ? `${green('unpinned')} ${server}`
      : dim(`${server} was not pinned`),
  );
}
...
export async function runMcpPins(opts: { json?: boolean }): Promise<void> {
  ...
  if (!pins.length)
    return console.log(
      dim('No pinned MCP servers. Pin one with `lurq mcp-pin <url-or-registry-name>`.'),
    );
  console.log(
    table(
      ['Server', 'Status', 'Since pinned'],
      pins.map((p) => [p.url, p.status ?? '—', pinLine(p)]),
    ),
  );
}
...
export async function runConnectCheck(
  server: string,
  opts: { client?: string; json?: boolean },
): Promise<void> {
  ...
  const res = await fromIndex<Response>('connect_check', args, (db) =>
    Promise.all([import('../connect/check'), import('../core/safeFetch')]).then(([m, f]) =>
      m.handleConnectCheck(
        db,
        { server, client: (opts.client ?? null) as Response['clients'][number]['client'] | null },
        { liveProbe: { fetch: f.sharedSafeFetch() } },
      ),
    ),
  );
  ...
  console.log(
    `${bold(res.server?.name ?? server)}${res.server?.url ? dim(`  ${res.server.url}`) : ''}`,
  );
  const auth = ev.auth
  ...
  for (const c of ev.recentChanges ?? [])
    console.log(dim(`· ${c.at.slice(0, 10)} ${c.kind}: ${c.summary}`));
  ...
  console.log(
    table(
      ...
      clients.map((c) => {
        ...
        return [
          c.clientName,
          paint[c.verdict](c.verdict.replace('_', ' ')),
          why.length > 100 ? `${why.slice(0, 99)}…` : why,
        ];
      }),
    ),
  );
  ...
  if (clients.length === 1) {
    ...
    for (const cfg of c.config ?? [])
      console.log(`\n${dim(`# ${cfg.kind} · ${cfg.target}`)}\n${cfg.text}`);
  } else if (clients.length > 1) {
    const s = res.summary;
    console.log(
      dim(
        `${s?.works ?? 0} works · ${s?.needs_setup ?? 0} needs setup · ${s?.blocked ?? 0} blocked · ${s?.unknown ?? 0} unknown  (--client <id> for steps and config)`,
      ),
    );
  }
...
export async function runMcpStack(
  ...
  // Live, from the user's own configuration: this used to need DATABASE_URL,
  // which a user with an API key does not have, and saw npm servers only.
  const { runMcpStackLive } = await import('./mcpScan');
  await runMcpStackLive(dir, opts);
}
