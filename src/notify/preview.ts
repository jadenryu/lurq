...
export function sampleUrgent(): UrgentItem[] {
  return [
    ...
    },
    {
      ...
      detail: 'The range >=17 already admits 19.0.0, a new major (it resolves 18.5.0 today). The next clean install takes it unless the range is tightened.',
...
export async function runNotifyPreview(opts: PreviewOptions): Promise<void> {
  ...
  const digest = renderDigest(sampleDigest(), { ...sampleLinks, unsubscribeUrl: sampleLinks.unsubscribeUrl.replace('urgent', 'digest') });
  ...
  const fmt = { deliveryId: 'channel:preview', dashboardUrl: `${WEB}/dashboard/notifications`, now: new Date(), signingSecret: 'preview-signing-secret' };
  ...
  console.log('Edit src/notify/render.ts (email) or src/notify/channels.ts (channels) and run this again.');
