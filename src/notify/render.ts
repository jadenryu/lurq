...
export function renderUrgent(items: UrgentItem[], links: Links): Rendered {
  const subject =
    items.length === 1
      ? `lurq: ${oneLine(items[0]!.title)}`
      : `lurq: ${items.length} urgent changes to what your agents depend on`;
  const intro =
    items.length === 1
      ? 'One change needs a look today.'
      : `${items.length} changes need a look today.`;
  const why =
    'You get this because urgent alerts are on for your lurq account. They only fire for changes like these.';
...
export function renderDigest(s: DigestSummary, links: Links): Rendered {
  ...
  const section = (title: string, rows: string) =>
    rows
      ? `<div style="margin-top:18px;font-size:12px;color:#8a8a8a;">${esc(title)}</div>${rows}`
      : '';
  const more = (shown: number, total: number) =>
    total > shown
      ? `<div style="padding-top:8px;font-size:12px;color:#6b6b6b;">and ${total - shown} more in the dashboard</div>`
      : '';
    ...
    [
      section(
        ...
        s.mcpChanges
          ...
          .join('') + more(s.mcpChanges.length, s.mcpChangeTotal),
      ),
      section(
        'Breaking releases',
        s.alerts.map((a) => row('', a.title, a.detail, a.url)).join('') +
          more(s.alerts.length, s.alertTotal),
      ),
      section(
        'Could not be read last scan',
        s.unreadable.map((u) => row('', u.alias, u.status.replace(/_/g, ' '), u.url)).join(''),
      ),
      section(
        'Not scanned in over a week',
        s.stale.map((u) => row('', u.alias, `last scanned ${u.days} days ago`, u.url)).join(''),
      ),
    ].join(''),
    footerHtml(why, links),
  );
  ...
  if (s.mcpChanges.length)
    lines.push(
      'MCP server changes:',
      ...s.mcpChanges.map((c) => `- [${c.severity}] ${c.alias}: ${c.summary} ${c.url}`),
      '',
    );
  if (s.alerts.length)
    lines.push('Breaking releases:', ...s.alerts.map((a) => `- ${a.title} ${a.url}`), '');
  if (s.unreadable.length)
    lines.push(
      'Could not be read last scan:',
      ...s.unreadable.map((u) => `- ${u.alias} (${u.status}) ${u.url}`),
      '',
    );
  if (s.stale.length)
    lines.push(
      'Not scanned in over a week:',
      ...s.stale.map((u) => `- ${u.alias}, ${u.days} days ${u.url}`),
      '',
    );
