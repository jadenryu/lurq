...
export function RuixenGradientFooter({
  ...
  // Strips every non-alphanumeric character rather than just ":". React's useId
  // format is not stable across majors (React 18 shipped `:r0:`, 19 changed it),
  // and anything exotic left in here lands inside `url(#grad-…)`, where one bad
  // character silently kills the fill for the whole band.
  ...
  useEffect(() => {
    ...
    // Scroll-linked growth is motion. Someone who asked for less of it gets the
    // band at its resting height and no listener at all.
    ...
    const measure = () => {
      ...
      // Written straight to the node. Routing this through useState re-renders
      // the whole footer on every scroll event, which is a lot of React for one
      // number that only ever reaches a transform.
