---
title: Quick start
description: Install lurq once, run setup once, and you're done.
---

lurq is a **hosted service** — you don't run a database or a sync to use it. You
install the CLI, run setup once, and it wires up both your terminal and every
...
Setup opens [the dashboard](https://lurq.run/dashboard/keys) in your browser so
you can sign in and create an API key, then walks you through pasting it back.
...
<Callout type="info">
  `lurq install` and `lurq login` both still work as aliases for `lurq setup`.
</Callout>
