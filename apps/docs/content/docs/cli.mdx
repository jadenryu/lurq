...
already-committed workflow is left alone unless you pass `--force`. It then
follows the runs it started and reports how they ended (`--no-watch` skips
that), because a setup command that reports success for a run that fails a
minute later is worse than one that reports nothing. Secrets go to
