...
# spell out the repository and how far it may go
lurq autopilot init --repo owner/name --mode pr

# fix mode needs no model and still opens pull requests — it is the default
lurq autopilot init --mode fix
```

It **never commits and never pushes.** Committing
`.github/workflows/lurq-upgrade.yml` is what grants write access to the
repository, so it writes the file and stops; you read the diff and commit it.
Secrets go to `gh` on stdin, never in arguments, so they stay out of `ps` and
your shell history.
