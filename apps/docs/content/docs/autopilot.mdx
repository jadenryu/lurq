...
That writes `.github/workflows/lurq-upgrade.yml` into the checkout you run it
in, detecting your package manager from the lockfile on disk. It refuses to
overwrite an existing file unless you pass `--force`, and it prints the
repository secrets to add. Nothing runs until you commit it.

The dashboard renders the same file per repository if you would rather copy it by
hand — **Repositories → your repo → workflow** — and the **copy setup for agent**
button there hands the whole job to a coding agent instead.
...
<Callout type="warn">
  **That token expires one year after you create it**, with no renewal and no
  warning. A weekly job runs fine until it lapses and then fails. An API key from
  the Anthropic console does not expire, and is what Anthropic recommends for a
  secret shared across repositories — an OAuth token belongs to whoever ran
  `claude setup-token`, and dies with their subscription.
</Callout>
