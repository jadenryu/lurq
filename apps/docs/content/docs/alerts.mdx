...
The emails are [React Email](https://react.email) templates. Run the preview
server and edit with live reload:

```bash
npm run email:dev     # http://localhost:3030
```

- `src/notify/emails/theme.ts` holds every colour, size and font.
- `src/notify/emails/components.tsx` is the shared layout, alert row and footer.
- `src/notify/emails/urgent.tsx` and `weekly.tsx` are the two emails, each
  previewed with the sample data in `samples.ts`.
- The subjects and plain-text parts are in `src/notify/render.ts`.

To check the plain-text parts and the Slack, Discord, Teams and webhook
payloads, or to send both emails to a real inbox:
...
The channel formats are `src/notify/channels.ts`.
