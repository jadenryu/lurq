...
import {
  failureProps,
  MAX_SERVERS_PER_UPLOAD,
  parseUpload,
  PUBLIC_QUORUM,
} from '../src/mcpScan/ingest';
...
describe('parseUpload', () => {
  it('accepts a well-formed upload and recomputes the hashes itself', () => {
    ...
    expect(c.contentHash).toBe(
      contentHash({ tools: [add()], prompts: [], resourceTemplates: [], instructions: null }),
    );
  ...
  it('rejects the whole body when its shape is wrong', () => {
    ...
    expect(
      parseUpload({ servers: Array.from({ length: MAX_SERVERS_PER_UPLOAD + 1 }, () => server()) })
        .error,
    ).toBeTruthy();
  ...
  it('rejects one bad server without dropping the rest', () => {
    const r = parseUpload({
      servers: [
        server(),
        server({ alias: 'bad', configFingerprint: 'not-hex' }),
        server({ alias: 'b2', serverKey: 'local:b2' }),
      ],
    });
  ...
  it('requires identity to be internally consistent', () => {
    ...
    expect(r.rejected.map((x) => x.reason)).toEqual([
      'serverKey does not match packageName',
      'serverKey must start with "local:"',
    ]);
  });
  ...
  it('refuses a readable status with no snapshot, and statuses that were never contacted', () => {
    const r = parseUpload({
      servers: [
        ...
        server({ alias: 'u', serverKey: 'local:u', status: 'untrusted' }),
      ],
    });
    expect(r.servers).toEqual([]);
  ...
  it('re-normalizes and re-scrubs whatever the client sent', () => {
    const r = parseUpload({
      servers: [
        server({}, [
          add('uses ghp_abcdefghijklmnopqrstuvwxyz0123'),
  ...
  it('keeps failure statuses, without a contract', () => {
    const r = parseUpload({
      servers: [server({ status: 'auth_required', error: 'HTTP 401', snapshot: null })],
    });
...
describe('failureProps', () => {
  it('keeps the class and code of a failure, never its message', () => {
    expect(
      failureProps(Object.assign(new TypeError('row (tools)=(secret)'), { code: '23505' })),
    ).toEqual({ error: 'TypeError', code: '23505' });
...
describe.skipIf(!TEST_DB)('ingestScan against Postgres', () => {
  ...
  afterAll(async () => {
    ...
    await db.execute(
      sql`delete from mcp_change_events where owner_id like ${`test_mcpscan_${run}_%`}`,
    );
    await db.execute(
      sql`delete from mcp_observations where owner_id like ${`test_mcpscan_${run}_%`}`,
    );
    await db.execute(
      sql`delete from mcp_deployments where owner_id like ${`test_mcpscan_${run}_%`}`,
    );
    await db.execute(
      sql`delete from mcp_public_reports where owner_id like ${`test_mcpscan_${run}_%`}`,
    );
    await close();
  });

  const ingest = (ownerId: string, servers: unknown[]) =>
    ingestScan(db, ownerId, parseUpload({ servers }));
  ...
  it('re-arms one event when a server flaps, rather than stacking duplicates', async () => {
    ...
    expect(
      (await store.getDeploymentDetail(db, o, first.servers[0]!.deploymentId))!.observations,
    ).toHaveLength(4);
  });
  ...
  it('never shows one account another account’s servers', async () => {
    ...
    expect(
      await store.getDeploymentDetail(db, owner('iso2'), mine.servers[0]!.deploymentId),
    ).toBeNull();
  ...
  it('reports a server that could not be recorded, without its error text', async () => {
    ...
    const spy = vi
      ...
      .mockRejectedValueOnce(
        Object.assign(new Error('duplicate key (owner_id)=(secret_value)'), { code: '23505' }),
      );
    vi.mocked(analytics.capture).mockClear();
    const r = await ingest(owner('evfail'), [server()]);
    expect(r.rejected).toEqual([
      expect.objectContaining({ reason: 'could not be recorded; try again' }),
    ]);
    const failed = vi
