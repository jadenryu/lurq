...
const msg = {
  ...
  html: '<p>h</p>',
  text: 't',
  idempotencyKey: 'urgent:user_1:abc',
...
describe('lookupEmail', () => {
  ...
  it('returns the verified primary address', async () => {
    ...
    expect(await lookupEmail('user_abc', { secretKey: 'sk', fetchImpl: fetchImpl as never })).toBe(
      'me@example.com',
    );
  });
  ...
  it('never uses an unverified primary, even when another address is verified', async () => {
    ...
    expect(
      await lookupEmail('user_abc', { secretKey: 'sk', fetchImpl: fetchImpl as never }),
    ).toBeNull();
  });
  ...
  it('returns null for a missing user or a non-user id, without calling Clerk for the latter', async () => {
    ...
    expect(
      await lookupEmail('user_gone', { secretKey: 'sk', fetchImpl: fetchImpl as never }),
    ).toBeNull();
    fetchImpl.mockClear();
    expect(
      await lookupEmail('org_team', { secretKey: 'sk', fetchImpl: fetchImpl as never }),
    ).toBeNull();
    expect(
      await lookupEmail('user_x/../admin', { secretKey: 'sk', fetchImpl: fetchImpl as never }),
    ).toBeNull();
  ...
  it('throws a retryable error when Clerk is down', async () => {
    ...
    const err = (await lookupEmail('user_abc', {
      secretKey: 'sk',
      fetchImpl: fetchImpl as never,
    }).catch((e) => e)) as SendError;
    expect(err.retryable).toBe(true);
