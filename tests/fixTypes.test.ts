...
describe('applyEdits', () => {
  ...
  it('is insensitive to the order edits arrive in', () => {
    const src = 'abcdef';
    const a: Edit[] = [
      edit({ start: 0, end: 1, text: 'A' }),
      edit({ start: 4, end: 5, text: 'E' }),
    ];
  ...
  it('collects every finding’s edits per file, so a writer touches each file once', () => {
    const byFile = editsByFile([
      finding({
        fix: {
          summary: 'one',
          edits: [edit({ file: 'src/a.ts' }), edit({ file: 'src/b.ts', start: 5, end: 6 })],
        },
      }),
...
describe('deterministic', () => {
  it('is true only for a finding that carries edits, never for an agent task', () => {
    ...
    expect(
      deterministic(
        finding({
          fix: { summary: 'x', task: { instruction: 'rewrite', files: [], evidence: [] } },
        }),
      ),
    ).toBe(false);
