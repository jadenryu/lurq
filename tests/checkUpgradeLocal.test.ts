...
function publish(name: string, version: string, manifest: object, files: Record<string, string>): void {
  ...
  registry.set(`https://registry.npmjs.org/${name}/${version}`, { json: { ...full, dist: { tarball, integrity } } });
...
async function check(root: string, pkg: string, fromVersion: string, toVersion: string): Promise<UpgradeReport> {
  return checkUpgrade([{ package: pkg, fromVersion, toVersion }], scanReferences(root), { rootDir: root });
}
...
describe('verdicts a CI gate acts on', () => {
  ...
  it('reads through bundler export getters instead of reporting every parameter gone', async () => {
    publish('lib', '1.0.0', { main: 'index.js' }, {
      'index.js': `"use strict";\nexports.parse = void 0;\nfunction parse(str, opts) { return str; }\nexports.parse = parse;\n`,
    });
    publish('lib', '1.1.0', { main: 'index.js' }, {
      'index.js': [
        `"use strict";`,
    ...
    const report = await check(project({ 'a.js': `import { parse } from 'lib';\nparse('x', {});\n` }), 'lib', '1.0.0', '1.1.0');
  ...
  it('follows a deep import that moved into a directory with its own package.json', async () => {
    publish('deep', '1.0.0', { main: 'index.js' }, { 'index.js': 'exports.x = 1;', 'sub.js': 'exports.a = function a() {};' });
    publish('deep', '1.1.0', { main: 'index.js' }, {
      'index.js': 'exports.x = 1;',
      'sub/package.json': JSON.stringify({ main: '../dist/sub.js' }),
      'dist/sub.js': 'exports.a = function a() {};',
    });
    const report = await check(project({ 'c.js': `const { a } = require('deep/sub');\na();\n` }), 'deep', '1.0.0', '1.1.0');
  ...
  it('blocks a withdrawn entry loaded only for its side effects, and names the Node it needs', async () => {
    publish('reg', '1.0.0', { main: 'index.js' }, { 'index.js': 'exports.x = 1;', 'register.js': 'require("./index")' });
    publish('reg', '2.0.0', { exports: { '.': './index.js' }, type: 'module', engines: { node: '>=24' } }, {
      'index.js': 'export const x = 1;',
    });
    const root = project({ 'package.json': JSON.stringify({ engines: { node: '>=18' } }), 'd.js': `require('reg/register');\n` });
  ...
  it('blocks the removal of a base class the code extends', async () => {
    publish('base', '1.0.0', { main: 'index.js' }, { 'index.js': 'exports.Emitter = class Emitter {}; exports.other = 1;' });
    publish('base', '2.0.0', { main: 'index.js' }, { 'index.js': 'exports.other = 1;' });
    const root = project({ 'e.js': `import { Emitter } from 'base';\nexport class Bus extends Emitter {}\n` });
    const [finding] = (await check(root, 'base', '1.0.0', '2.0.0')).breaking;
    expect(finding).toMatchObject({ severity: 'blocking', symbolsRemoved: [{ symbol: 'Emitter' }] });
  });
  ...
  it('does not count a same-named JSX attribute as an unmeasurable use', async () => {
    publish('ui', '1.0.0', { main: 'index.js' }, { 'index.js': 'exports.render = function render(a) {};' });
    publish('ui', '2.0.0', { main: 'index.js' }, { 'index.js': 'exports.render = function render(a, b) {};' });
    const root = project({
  ...
  it('checks Node and peer requirements for a package the code never imports', async () => {
    ...
    const root = project({ 'package.json': JSON.stringify({ engines: { node: '>=18' } }), 'x.js': '' });
    const [finding] = (await check(root, 'tool', '1.0.0', '2.0.0')).breaking;
    expect(finding).toMatchObject({ severity: 'warning', requirements: [{ name: 'node', needs: '>=24' }] });
  });

  // A parameter that shadows the binding is a different variable.
  it('never blocks on a call of a shadowed name', async () => {
    publish('paint', '1.0.0', { main: 'index.js' }, { 'index.js': 'module.exports = function paint(s) { return s; };' });
    publish('paint', '2.0.0', { type: 'module', exports: './index.js' }, { 'index.js': 'export default function paint(s) { return s; }' });
    const root = project({
      'package.json': JSON.stringify({ engines: { node: '>=22.12' } }),
