...
vi.mock('../src/db/notifications', () => ({
  ...
  setPreferences: vi.fn(async (_db: unknown, _o: string, patch: Record<string, boolean>) => ({
    urgentEmail: true,
    weeklyDigest: false,
    ...patch,
  })),
...
beforeAll(async () => {
  ...
  registerNotificationRoutes(app, {
    db: {} as never,
    requireIssuerSecret: (req: Request, res: Response, next: NextFunction) =>
...
const call = (method: string, path: string, body?: unknown, issuer = 'ok') =>
  fetch(`${base}${path}`, {
    method,
    headers: { 'x-issuer': issuer, 'content-type': 'application/json' },
...
describe('notification preferences', () => {
  it('reads the defaults, and whether email is configured at all', async () => {
    expect(
      (await call('GET', '/notification-preferences?ownerId=user_1', undefined, 'bad')).status,
    ).toBe(401);
  ...
  it('writes only booleans it knows', async () => {
    expect(
      (await call('PUT', '/notification-preferences', { ownerId: 'user_1', weeklyDigest: 'yes' }))
        .status,
    ).toBe(400);
    expect(
      (await call('PUT', '/notification-preferences', { ownerId: 'user_1', admin: true })).status,
    ).toBe(400);
    const ok = await call('PUT', '/notification-preferences', {
      ownerId: 'user_1',
      weeklyDigest: true,
      admin: true,
    });
...
describe('unsubscribe', () => {
  it('answers the same for a matching and an unknown token', async () => {
    const known = await call('POST', '/notifications/unsubscribe', {
      token: 'a'.repeat(43),
      kind: 'urgent',
    });
    const unknown = await call('POST', '/notifications/unsubscribe', {
      token: 'b'.repeat(43),
      kind: 'urgent',
    });
  ...
  it('refuses a malformed token or kind', async () => {
    expect(
      (await call('POST', '/notifications/unsubscribe', { token: 'short', kind: 'urgent' })).status,
    ).toBe(400);
    expect(
      (
        await call('POST', '/notifications/unsubscribe', {
          token: 'a'.repeat(43),
          kind: 'everything',
        })
      ).status,
