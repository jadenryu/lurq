...
describe('parseWwwAuthenticate', () => {
  it('reads quoted, bare and escaped params, case-insensitively', () => {
    expect(
      parseWwwAuthenticate(
        'Bearer resource_metadata="https://mcp.x.dev/.well-known/oauth-protected-resource", scope="files:read files:write"',
      ),
    ).toEqual({
      scheme: 'bearer',
      params: {
        resource_metadata: 'https://mcp.x.dev/.well-known/oauth-protected-resource',
...
describe('discovery URL order', () => {
  it('tries the header, then the path-inserted well-known, then the root', () => {
    expect(resourceMetadataCandidates(new URL('https://x.dev/public/mcp'), '/prm')).toEqual([
      ...
      {
        url: 'https://x.dev/.well-known/oauth-protected-resource/public/mcp',
        via: 'well_known_path',
      },
...
describe('discoverOAuth', () => {
  it('reads a compliant server end to end, with no violations', async () => {
    ...
    const r = await discoverOAuth(
      fetch,
      EP,
      parseWwwAuthenticate(`Bearer resource_metadata="${PRM}", scope="read"`),
    );
  ...
  it('finds metadata at the well-known path when the header omits it, and flags a strict-client failure set', async () => {
    ...
    expect(r.violations.find((v) => v.code === 'issuer_mismatch')!.detail).toMatch(
      /trailing slash/,
    );
  });
  ...
  it('flags a relative resource_metadata URL but still follows it', async () => {
    const fetch = docs({
      ...
      [AS]: {
        issuer: 'https://auth.x.dev',
        registration_endpoint: 'https://auth.x.dev/r',
        code_challenge_methods_supported: ['S256'],
      },
    });
    const r = await discoverOAuth(
      fetch,
      EP,
      parseWwwAuthenticate('Bearer resource_metadata="/prm.json"'),
    );
  ...
  it('reports metadata that names a different resource, a non-string resource, and an unreachable AS', async () => {
    const mismatch = await discoverOAuth(
      docs({
        [PRM]: { resource: 'https://other.dev', authorization_servers: ['https://auth.x.dev'] },
      }),
      EP,
      null,
    );
    expect(mismatch.violations.map((v) => v.code)).toEqual([
      'resource_mismatch',
      'as_metadata_unreachable',
    ]);
    const listResource = await discoverOAuth(
      docs({
        [PRM]: { resource: ['https://mcp.x.dev'], authorization_servers: ['https://auth.x.dev'] },
      }),
      EP,
      null,
    );
  ...
  it('separates an unreachable header URL from an invalid document', async () => {
    const unreachable = await discoverOAuth(
      docs({}),
      EP,
      parseWwwAuthenticate(`Bearer resource_metadata="${PRM}"`),
    );
