...
describe.skipIf(!TEST_DB)('public MCP change routing', () => {
  ...
  beforeAll(async () => {
    ...
    for (const owner of [scanner, both]) {
      await db.insert(mcpDeployments).values({
        ...
        alias: owner === scanner ? 'tools' : 'my-tools',
        registry: 'remote',
        packageName: null,
        transport: 'http',
        lastStatus: 'ok',
  ...
  afterAll(async () => {
    ...
    await db.execute(
      sql`delete from mcp_endpoint_change_acks where owner_id like ${`test_pubalert_${run}_%`}`,
    );
    await db.execute(sql`delete from mcp_pins where owner_id like ${`test_pubalert_${run}_%`}`);
    await db.execute(
      sql`delete from mcp_deployments where owner_id like ${`test_pubalert_${run}_%`}`,
    );
  ...
  it('pins the contract and sign-in path as they are now', async () => {
    ...
    expect((await alerts.listPins(db, pinner))[0]).toMatchObject({
