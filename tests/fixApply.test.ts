...
const importEdit = (file: string): Edit => ({ file, start: 9, end: 14, text: 'parseCookie', was: 'parse' });
...
describe('stage', () => {
  it('returns the new contents for every file, and writes none of them', () => {
    const { writes, diff } = stage(root, [finding([importEdit('src/a.ts'), importEdit('src/b.ts')])]);
  ...
  it('refuses the whole batch when one file moved since it was read', () => {
    ...
    write('src/b.ts', `import {parse} from 'cookie';\nexport const other = (h: string) => parse(h);\n`);
  ...
  it('ignores findings that carry no edits, so a brief never reaches the writer', () => {
    const brief: Finding = {
      ...
      fix: { summary: 'ask the agent', task: { instruction: 'rewrite it', files: ['src/a.ts'], evidence: [] } },
    };
