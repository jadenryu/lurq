...
describe('endpointIdentity', () => {
  it('folds only what URLs define as equivalent', () => {
    expect(endpointIdentity('HTTPS://Mcp.Acme.DEV:443/mcp#frag')).toEqual({
    ...
    expect(endpointIdentity('https://mcp.acme.dev/mcp?tenant=A')!.url).toBe(
      'https://mcp.acme.dev/mcp?tenant=A',
    );
    expect(endpointIdentity('https://mcp.acme.dev:8443/mcp')!.url).toBe(
      'https://mcp.acme.dev:8443/mcp',
    );
  });

  it('marks templated URLs and keeps them verbatim', () => {
    expect(endpointIdentity('https://{tenant}.acme.dev/mcp')).toEqual({
      url: 'https://{tenant}.acme.dev/mcp',
      host: 'placeholder.acme.dev',
      templated: true,
    });
