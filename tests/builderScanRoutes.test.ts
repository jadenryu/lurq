...
const population = [
  ...
  { login: 'ada', repos: 999, active90: 0, stars: 0, depsTracked: 0, depsBehind: 0, depsMajor: 0, advisories: 0 },
];
...
vi.mock('../src/db/builderScans', () => ({
  ...
  getBuilderScan: vi.fn(async (_db: unknown, ownerId: string, target: string) => {
    ...
    return row
      ? { ...row, metrics: { repos: 3, active90: 2, stars: 10, depsTracked: 0, depsBehind: 0, depsMajor: 0, advisories: 0 } }
      : null;
  }),
...
describe('saved builder scans', () => {
  ...
  it('answers a save and a reopen with the same standing, never counting the builder against themselves', async () => {
    const saved = (await (await call('PUT', '/builder-scans', { ownerId: 'user_1', target: 'ada', profile })).json()) as { standing: Standing };
    const one = (await (await call('GET', '/builder-scans/one?ownerId=user_1&target=ada')).json()) as { scan: { standing: Standing } };
