...
- **2026-06-20 — R1 (build kickoff refinements).** Captured before/during M0–M1:
  ...
  13. **Diagram tool + polish (M8):** The optional `diagram` tool emits a layered Mermaid reference diagram from a `stack` (package names, categories resolved from the DB) or a `description` (multi-category keyword inference). Known backend frameworks (express/fastify/koa/hono/nest/hapi) are routed to a Backend layer since category `framework` alone can't distinguish front/back — the one place the diagram needs a hint, consistent with it being a labeled starting point, not an oracle. README extended with setup/usage/tools/install docs (preserving the owner's positioning + outreach). **All §19 acceptance criteria pass** (migrate+sync populates real scores; serve speaks MCP; recommend ≤5 with confidence+dataAsOf in ~370 tokens; verify distinguishes real/typosquat/nonexistent; on-demand fetch works; single-source outages tolerated; 75 tests pass).
  12. **Skill installer notes (M7):** `install-skill` supports claude-code (default), cursor, windsurf, copilot (VS Code, uses the `servers` key + `type: stdio`), **codex** (TOML `[mcp_servers.lurq]`), and `all` (installs only to detected agents). It **merges** a single `lurq` entry — verified non-destructive (preserves other servers + unrelated keys) and idempotent — populates the `env` block from the current environment (DATABASE_URL etc.), drops the skill-instructions template to `~/.lurq/`, and prints exactly what it wrote + next steps. MCP config paths/formats move fast and the claude-code-guide agent couldn't confirm them live, so the installer is deliberately transparent (reports every path) rather than silent, making any stale path obvious.
  11. **MCP server notes (M5):** Built on `@modelcontextprotocol/sdk` v1.x (`registerTool` + `StdioServerTransport`), zod-validated inputs, compact JSON-text outputs (evaluate ~960 chars). On-demand fetch (§12.5) works: querying an untracked-but-real package (e.g. `tiny-invariant`) fetches+scores+stores it. `verify` is a strong anti-typosquat guard — live-tested, it flags `lodahs` (a real lodash typosquat) with `low-downloads` + `has-known-advisory`, and returns `not-found-on-registry` for hallucinated names. On-demand packages outside the seed get `category: null` (no classifier in v1), so they're searchable by embedding but not category-filtered.
  10. **Embeddings + search notes (M4):** (a) Embeddings are computed on ingest from the §8.2 text blob; the **local no-key embedder** is a hashing bag-of-words (lexical overlap) — real but rough — while OpenAI gives true semantics. Both emit 1536-dim unit vectors. (b) The re-rank (§11) normalizes cosine with a **fixed `(sim+1)/2` transform, not pool min-max** — min-max over-amplified the single closest match, letting one noisy local-embedding hit outrank a much healthier package (e.g. moment topping a dates query). Fixed normalization keeps the `0.4*health` term meaningful; live results are now health-respecting (dayjs/date-fns above moment, typeorm/prisma/drizzle for ORM). (c) Category inference is keyword-based and returns null when uncertain (search-all); the search broadens automatically if a category filter starves results.
